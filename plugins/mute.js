...
const command = ["mute"], description = "không cho người này dùng bot"
	...
	async call (data,db,author,victim,avt) {
		...
		if (data.guild) {
			if (id == data.guild.ownerId || config.admin.includes(id)) {
				...
				if (db.user[mute].mutesv) return data.reply({embeds:[defaultemb(`người dùng ${db.user[mute].tag} hiện không được sử dụng bot tại máy chủ này`)]})
				db.user[mute].mutesv = true
				return data.reply({embeds: [defaultemb(`người dùng: ${db.user[mute].tag}\nđã bị mute và không thể sử dụng bot tại máy chủ này`).setThumbnail((avt?avt:data.author).displayAvatarURL({size: 1024, dynamic: true}))]})
			...
			if (!mute) return data.reply("sai id")
			if (db.user[mute].mute) return data.reply({embeds:[defaultemb(`người dùng ${db.user[mute].tag} hiện không được sử dụng bot này`)]})
